...
**Where it lives**
`services/cache.py` — the deliberately naive exact-key version, kept as the strawman. `services/semantic_cache.py` — the real one: entries stored as Redis hashes (question, `source` TAG, answer JSON, 384-dim FLOAT32 embedding) behind a RediSearch `FLAT` cosine index, with `SIMILARITY_THRESHOLD = 0.92` and a 1-hour TTL. Both the stored embedding and the query embedding go through `normalize_question()` first — casefold, collapse whitespace, strip trailing punctuation — before `embed_text()` ever sees them. `services/graph.py::answer_question_cached()` is the cache-aside wrapper that `routers/query.py` actually calls.
...
A third gotcha showed up from something as trivial as punctuation. *"What is the refund window"* and *"What is the refund window?"* are the same question to a human, but a single trailing `?` moved cosine similarity by **0.065** — nearly the *entire* 0.068-wide window between the paraphrase (0.94) and the different-answer question (0.87) documented above. Left alone, that meant an unpunctuated paraphrase could miss the cache while a punctuated one hit it, for no reason connected to meaning at all. The fix is `normalize_question()` — casefold, collapse whitespace, strip trailing punctuation — applied before embedding, on both the write path (`set_cached_answer`) and the read path (`get_cached_answer`), so the same question with different formatting always maps to the same key. Measured effect: the safe threshold window widened from 0.068 to 0.096, about 41% more room between "must accept" and "must reject." The general lesson: a semantic key is *more* sensitive to surface noise than an exact key, not less, because formatting differences and meaning differences both move the same number.
...
- A trailing `?` alone moves similarity by 0.065, almost the whole safe window. Why does a semantic cache care about formatting noise that an exact-key cache would never even notice?
